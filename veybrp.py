...
def func (aa):
    return (0.398942280401433*(1 + 1/aa)**(-1.0 - 2/aa)*(1 + 2/aa)**(0.5 + 2/aa)*(1 + 0.0833333333333333/(1 + 2/aa) + 0.00347222222222222/(1 + 2/aa)**2 - 0.00268132716049383/(1 + 2/aa)**3 - 0.000229472093621399/(1 + 2/aa)**4)*exp(-1 - 2/aa)*exp(2 + 2/aa)/(1 + 0.0833333333333333/(1 + 1/aa) + 0.00347222222222222/(1 + 1/aa)**2 - 0.00268132716049383/(1 + 1/aa)**3 - 0.000229472093621399/(1 + 1/aa)**4)**2)
xmin = 0.5
xmax = 6
dx = 0.01
xlist = np.arange (xmin, xmax, dx)
ylist = [func(aa) for aa in xlist]
x=[float(i) for i in xlist]
y=[float(i) for i in ylist]
plt.plot (xlist, ylist)
plt.ylabel('d')
plt.xlabel('a')
plt.close()
interp_func = scipy.interpolate.interp1d(y, x)
a = interp_func(1.287550650795848)
